...
> **Reference building lives in a separate `preditr_ref` repository.**
...
> Prebuilt reference images for every supported organism are published on Docker Hub at
> [`fvasquezcastro/preditr-ref`](https://hub.docker.com/r/fvasquezcastro/preditr-ref) —
> you do not need the build tooling to run PrEditR.
...
output files for troubleshooting. Issues specific to building or publishing organism
reference images / payloads are tracked in the separate `preditr_ref` project; the
prebuilt images themselves are on
