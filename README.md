...
**Note:** At least one of **gene symbol**, **Ensembl transcript ID**, or **UniProt ID** is **REQUIRED** per target. Any one is sufficient. When more than one is given, the Ensembl transcript ID takes precedence over the UniProt ID, which takes precedence over the gene symbol. A gene symbol on its own resolves to the gene's canonical transcript.
