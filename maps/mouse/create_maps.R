...
ensembl_ids <- as.vector(read.table("maps/mouse/ensembl_in_txdb.txt")$V1)

biomart_export <- read.table(
  gzfile("maps/mouse/biomart_export_20251120.txt.gz"),
  header = TRUE, sep = "\t"
)
...
# Save RDS
saveRDS(ensembl_to_uniprot, "maps/mouse/ensembl_to_uniprot.rds", compress = "xz")
...
fwrite(
  ...
  "maps/mouse/ensembl_to_uniprot.txt.gz",
...
#Default to Ensembl canonical if no isoform specified
...
# Save the RDS
saveRDS(uniprot_to_ensembl, "maps/mouse/uniprot_to_ensembl.rds", compress = "xz")
...
saveRDS(has_isoforms, "maps/mouse/has_isoforms.rds", compress = "xz")
...
saveRDS(duplicated_uniprot, file = "maps/mouse/duplicated_uniprot.rds")
