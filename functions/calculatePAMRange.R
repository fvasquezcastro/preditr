calculatePAMRange <- function(edit_type, editor, codon_absolute_locations, codon_seq, strand){
  ...
  if (strand == "+"){ #SCENARIO A: Gene found on the positive strand
    ...
    for (l in 1:3){
        ...
        #Guides on the positive strand
      
        current_strand_base <- substr(codon_seq, l, l)
        
        if (current_strand_base == target_base){
        ...
        #Guides on the negative strand

        opposite_strand_base <- substr(comp_codon_seq, l, l)
        
        if (opposite_strand_base == target_base){
