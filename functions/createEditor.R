createEditor <- function(editor_name, pam, spacer_length, edit_type, edit_window_min, edit_window_max){
    ...
    nuclease <- crisprBase::CrisprNuclease(paste0(editor_name, "_nuclease"),
                               targetType = "DNA",
  ...
  editor <- crisprBase::BaseEditor(nuclease, baseEditorName=editor_name, editingWeights = weights)
