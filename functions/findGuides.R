findGuides <- function(cds_coordinates, genome, txdb, editor, absolute_codon_locations, 
  ...
  range <- IRanges::IRanges(start=min(absolute_codon_locations)-spacer_length,
                  end=max(absolute_codon_locations)+spacer_length)
  
  granges <- GenomicRanges::GRanges(seqnames = as.character(cds_coordinates@seqnames)[1], ranges = range,
                     strand = as(strand, "Rle"), seqinfo = NULL, seqlengths = NULL)
  
  
  guides <- crisprDesign::findSpacers(granges, 
                        bsgenome = genome,
                        crisprNuclease = get(editor))
  
  guides <- crisprDesign::addSequenceFeatures(guides)
  
  
  pam_ranges <- calculatePAMRange(edit_type, editor, absolute_codon_locations, codon_seq, strand)
  
  
  candidate_rows <- c() #Holds indices, not sequences
  ...
  candidate_guides <- guides[unique(candidate_rows)]
  
  #Guides that are non-editing can make it up to this point, but only editing guides will be returned
  ...
  spacer_len <- get(editor)@spacer_length
  
  nec_mask <- mapply(
    isNEC,
    as.character(as.vector(candidate_guides$protospacer)),
    MoreArgs = list(target_base = toupper(substr(edit_type, 1,1)),
                    editing_window = editing_window,
                    spacer_len = spacer_len)
  )
  
  candidate_guides <- candidate_guides[unname(!nec_mask)]
  
   #Only the guides whose PAM site is located within the optimal edit range to target the desired codon in the desired base
