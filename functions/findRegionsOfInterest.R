findRegionsOfInterest <- function(all_coordinates, codon_absolute_locations, editor){
  ...
  intron_regions <- sort(as.integer(stringr::str_extract(stringr::str_extract(all_regions, "region_\\d+_intron"), "\\d+")))
  exon_regions <- sort(as.integer(stringr::str_extract(stringr::str_extract(all_regions, "region_\\d+_exon"), "\\d+")))
  ...
  codon_start_region <- which((all_coordinates@ranges@start <= codon_start & codon_start <= BiocGenerics::end(all_coordinates@ranges)))
  codon_end_region <- which((all_coordinates@ranges@start <= codon_end & codon_end <= BiocGenerics::end(all_coordinates@ranges))) 
  ...
  if (codon_start_region == codon_end_region){
    ...
    current_exon <- as.integer(stringr::str_extract(names(all_coordinates[codon_start_region, ]), "\\d+"))
    ...
    if (current_exon == 1){ #Then just consider the following intron, which is the same number but for intron
      ...
      previous_intron_end <- as.integer(BiocGenerics::end(all_coordinates[names(all_coordinates) == paste0("region_", current_exon-1, "_intron"), ]@ranges))
    ...
    exon1 <-  as.integer(stringr::str_extract(names(all_coordinates[codon_start_region, ]), "\\d+"))
    exon2 <- as.integer(stringr::str_extract(names(all_coordinates[codon_end_region, ]), "\\d+"))
