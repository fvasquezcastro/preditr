findOffTargets <- function(results, genome, indexed_genome, n_mismatches, n_max_alignments, txdb){
  ...
  if (length(unique(prefixes)) != 1){
    #This should be caught when checking then input
  ...
  suppressWarnings(
    guideSet <- crisprDesign::addSpacerAlignments(guideSet,
                                    txObject=txdb, 
  ...
  for (m in 0:n_mismatches){
    ...
    offtargets[[col_name]] <- as.character(S4Vectors::mcols(guideSet)[[c]])
