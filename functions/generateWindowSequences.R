generateWindowSeqs <- function(genome, cds_coordinates, target_position, strand, 
  ...
  coding_sequence <- paste0(Biostrings::getSeq(genome, cds_coordinates), collapse="")
  coding_sequence_ob <- Biostrings::DNAString(x=coding_sequence, start=1, nchar=NA)
  protein_seq <- Biostrings::translate(coding_sequence_ob, genetic.code=Biostrings::GENETIC_CODE, no.init.codon = FALSE)
  ...
  dna_flanking_seq <- Biostrings::DNAString(x=substr(coding_sequence, flanking_seq_min, flanking_seq_max), start=1, nchar=NA)
  

  #Same AA flanking string for all
  wildtype_flanking_aa <- as.character(translate(dna_flanking_seq, 
                                                 genetic.code = Biostrings::GENETIC_CODE, no.init.codon = TRUE))
  ...
  for (g in 1:length(candidate_guides)){
    ...
    if (same_side){
      ...
      guide <- Biostrings::reverseComplement(candidate_guides[g]$protospacer)
      ...
      if (same_side){
        ...
        wildtype_allele_seq <- as.character(Biostrings::complement(Biostrings::DNAString(x = substr(as.character(dna_flanking_seq), wildtype_allele_window[1], wildtype_allele_window[2]),
                                         start = 1, nchar = NA)))
      }
      ...
      mutant_aa_flanking_seq <- as.character(Biostrings::translate(Biostrings::DNAString(x=mutant_dna_flanking_seq, start=1, nchar=NA), 
                                            genetic.code = Biostrings::GENETIC_CODE, no.init.codon = TRUE))
