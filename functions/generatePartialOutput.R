generatePartialOutput <- function(row_num, candidate_guides, genome, target_position, target_aa,
  ...
  if (length(candidate_guides) == 0 | typeof(candidate_guides) == "NULL"){
    ...
    guide_strand <- as.character(candidate_guides[i]@strand)
    coords <- calculateGuideCoordinates(
      pam_site = candidate_guides[i]$pam_site,
      strand = guide_strand,
      spacer_len = nchar(as.character(candidate_guides[i]$protospacer)),
      pam_len = nchar(as.character(candidate_guides[i]$pam))
    )
    ...
    new_row <- data.frame(query_num = as.character(row_num),
                            ...
                            protospacer_strand = guide_strand,
                            pam_seq = as.character(candidate_guides[i]$pam),
                            chromosome = as.character(candidate_guides[i]@seqnames[1]),
                            pam_coordinates_start = as.character(coords$pam_start),
                            pam_coordinates_end = as.character(coords$pam_end),
                            protospacer_coordinates_start = as.character(coords$protospacer_start),
                            protospacer_coordinates_end = as.character(coords$protospacer_end),
                            polyA = as.character(candidate_guides[i]$polyA),
                            polyC = as.character(candidate_guides[i]$polyC),
                            polyG = as.character(candidate_guides[i]$polyG),
                            polyT = as.character(candidate_guides[i]$polyT),
                            startingGGGGG = as.character(candidate_guides[i]$startingGGGGG),
