findRelativeTargetBasePosition <- function(target_aa, edit_type, strand, codon_seq){
  ...
  if (strand == "+"){
    ...
    codon_complement <- as.character(Biostrings::complement(Biostrings::DNAString(x=codon_seq, start=1, nchar=NA))) #For edits on the opposite strand that force the cellular machinery to correct the current strand
    ...
    if (grepl(target_letter, codon_seq)){ 
      ...
      for (e in edited_codons){
        translated_codons <- c(translated_codons, as.character(Biostrings::translate(Biostrings::DNAString(x=e, start=1, nchar=NA), genetic.code=Biostrings::GENETIC_CODE, no.init.codon = FALSE)))
    ...
    if (grepl(target_letter, codon_complement)){
      ...
      for (e in edited_codons_complement){
        ...
        translated_complement_codons <- c(translated_complement_codons, as.character(Biostrings::translate(Biostrings::complement(Biostrings::DNAString(x=e, start=1, nchar=NA)), genetic.code = Biostrings::GENETIC_CODE,  no.init.codon = FALSE)))
  ...
  if (strand == "-"){
    ...
    reverse_codon_seq <- Biostrings::reverse(codon_seq) #Because it is passed as the 5'-3' sequence even if it is on the 3'-5' end. It is only reversed, not "reverse-complemented". For example, for a tyrosine TAC -> CAT
    ...
    if (grepl(target_letter, reverse_codon_seq)){ #For edits on the current strand, which in this case is the negative one
      ...
      for (r in reverse_codons_edited){
        ...
        translated_reverse_codons <- c(translated_reverse_codons, as.character(Biostrings::translate(Biostrings::DNAString(x=Biostrings::reverse(r), start=1, nchar=NA), genetic.code=Biostrings::GENETIC_CODE, no.init.codon = FALSE))) #The codons must be reversed before the translation since the translation table is in the 5'-3' direction
    ...
    complement_reverse_codon_seq <- as.character(Biostrings::complement(Biostrings::DNAString(x=reverse_codon_seq, start=1, nchar=NA)))
    ...
    if (grepl(target_letter, complement_reverse_codon_seq)){
      ...
      for (cr in complement_reverse_codons_edited){
        ...
        translated_complement_reverse_codons <- c(translated_complement_reverse_codons, as.character(Biostrings::translate(Biostrings::complement(Biostrings::DNAString(x=Biostrings::reverse(cr), start=1, nchar=NA)), genetic.code = Biostrings::GENETIC_CODE,  no.init.codon = FALSE)))
      }
