...
findPotentialACL <- function(genome, ensembl_id, target_position, txdb) {
  ...
  cds_coordinates <- queryTxObject(txObject=txdb, #Find cds_coordinates for a specific gene
                               featureType="cds",
                               queryColumn="tx_id",
  ...
  if (unique(cds_coordinates@strand) == "-"){
    ...
    return(list(
      potentialACL=c(codon_absolute_end, codon_absolute_middle, codon_absolute_start), #It is really start, middle, end because the cds_coordinates in the neg stand are opposite. In summary, start < middle < end, and the numbers increase in the 5' direction 
      cds_coordinates = cds_coordinates,
      strand = strand,
      gene_symbol = gene_symbol
    ))
