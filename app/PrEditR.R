...
worker_fun <- function(row_num, gene_symbol, ensembl_id, target_aa, target_position, editor, edit_type) {
  
  if (debug){
    #Create separate directory in case it needs to dump the rda file
    dump_directory <- file.path(tmp_folder, preditr_temp, paste0("row_", row_num))
  ...
  tryCatchLog({
      ...
      if (shiny) {
        logInfo(paste0("Worker ", row_num, ": Loading organism data and editors."))
        suppressMessages(
          organism_data <- loadOrganismData(organism)
        )
        genome <- organism_data$genome
      ...
      logInfo(paste0("Worker ", row_num, ": Starting process."))
      
      processed_res <- process_row(
        row_num, gene_symbol, ensembl_id, target_aa, target_position, editor, edit_type,
        genome, indexed_genome, organism, txdb, n_mismatches,
        n_max_alignments, flanking5, flanking3, tmp_folder
      )
      
      logInfo(paste0("Worker ", row_num, ": Finished process."))
    ...
    error = function(e) {
      ...
      logError(paste0("Worker ", row_num, ": An unexpected error occurred."))
      error_res <- generateErrorOutput(row_num, n_mismatches)
...
#Run
results <- future_pmap(
  .l = list(df$row_num, df$gene_symbol, df$ensembl_id, df$target_aa, df$target_position, df$editor, df$edit_type),
  .f = worker_fun
)
